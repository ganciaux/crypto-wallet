...
const getCurrencyTotal = (currency, filter={}) => {
    const total={crypto:0.00, euro:0.00}
    ...
    currency.operations.forEach((operation) => {
        ...
        if (sum===true){
            if (operation['Currency']===currency.label){
                ...
                if (operation['To Currency'].length>0 && operation['To Currency']!==currency.label && formatValue(operation['Amount'])<0){
...
const read = (file) => {
    let headers = [];
    let result={operations:[],currencies:[],transactionKind:[],total_purchase:0.00}
    ...
    lines.forEach((line, index) => {
        if (line.length > 0) {
            ...
            if (index === 0) {
                ...
                const cell = cells.reduce((acc, cur, index) => ({ ...acc, [headers[index]]: cur }), {});
    ...
    result.currencies.forEach((currency) => {
        currency.total=getCurrencyTotal(currency, {key:'Transaction Kind', value:'lockup_lock'})
    });

    result.total_purchase=getTotal(result.operations, 'Native Amount', {key:'Transaction Kind', value:'crypto_purchase'});
...
const data = read('/data/crypto_transactions_record.csv');

write('/data/currencies.json', data);